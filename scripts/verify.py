...
check("binary.txt values are the 11 bit index", [r[0] for r in rows] == [format(i, "011b") for i in range(2048)])
check("binary.txt words match", [r[1] for r in rows] == words)

rows = [ln.split() for ln in read("wordlists/txt/bip-39-index-binary.txt")]
